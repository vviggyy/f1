...
#note 4 spaces for indent here
sed -i '643c\        reward = 0.1 * (self.__calculate_reward_laptime(t_pitdrive_inlap))' "$obj_path" #replace entire line with this
sed -i '653c\            #reward += self.__calculate_reward_final_position()' "$obj_path" #eliminate final position bonus
...
python main_racesim.py > "$OUTPUT" #check main racesim.py for pars ini file...

#replace (undo transformation)
sed -i '643c\        reward = 0.1 * (self.__calculate_reward_laptime(t_pitdrive_inlap) + self.__calculate_reward_position(delta_position))' "$obj_path" #replace entire line with this
sed -i '653c\            reward += self.__calculate_reward_final_position()' "$obj_path"
